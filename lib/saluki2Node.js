...
class Saluki2Client {
  ...
  async initServices() {
    ...
    try {
      ...
      for (const serviceName of Object.keys(this.serviceConfig)) {
        ...
        let grpcConstructor = this.protos[npmPackageName.split('-')[0]]
        ...
        if (!grpcConstructor) {
          if (!notInstalledProto.get(applicationName)) {
            ...
            console.warn(`${applicationName}: proto is not found. You may install it by: cnpm install @quancheng/${npmPackageName} --save`)
          }
          continue
        }
  ...
  _getClientInstance(appId, serviceName, grpcConstructor) {
    ...
    client = new grpcConstructor(`${instance.hostName}:${instance.port.$ + 1}`, this.combined_creds, grpcOptions)
  ...
  async _loadPem() {
    const pem = await fs.readFile(this.config.saluki2.pem || join(__dirname, './../resource/server.pem'))
  ...
  async _loadProto() {
    ...
    const pbPaths = await globby.sync(join(serviceAllFolder, '**/src/main/proto/**/*_service.proto'))
    for (const path of pbPaths) {
      ...
      try {
        ...
        this.protos[npmPkgName] = defaultsDeep(this.protos[npmPkgName], serviceProto)
      } catch (e) {
        console.error(e)
      }
    }
