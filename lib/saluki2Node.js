...
const { defaultsDeep, random, isString, isObject, get } = require('lodash')
...
class Saluki2Client {
  ...
  async initServices() {
    ...
    try {
      ...
      for (const serviceName of Object.keys(this.serviceConfig)) {
        ...
        let grpcConstructor =
          this.protos[source][applicationName.split('-')[0]] ||
          this.protos[source][packageNameArr[2]]
  ...
  _callService(service, methodName) {
    ...
    return (req = {}, metadata = {}) => {
      let customeMetadata = new grpc.Metadata()

      Object.keys(metadata).forEach(m => {
        customeMetadata.set(m, metadata[m])
      })
      ...
      return new Promise((resolve, reject) => {
        ...
        client.client[methodName](req, customeMetadata, (err, resp) => {
          customeMetadata = null
          if (err) {
            console.error(
              'request the service: ' +
                serviceName +
